...
class SubscriptionManager:
    def __init__(
        self,
        user_subscriptions: Dict[str, Set[Tuple[str, str]]] = None,
        subscribe_callback: Callable[[str, str, str], Awaitable[bool]] = None,
        unsubscribe_callback: Callable[[str, str, str], Awaitable[bool]] = None,
    ...
    async def add_user_subscription(
        self, user_id: str, symbol: str, subscription_type: str = "trades"
    ) -> bool:
        """
        ...
        if self.subscribe_callback:
            ...
            if success:
                ...
                self.user_subscriptions.setdefault(user_id, set()).add(
                    (symbol, subscription_type)
                )
                logger.info(
                    f"User {user_id} subscribed to {symbol} ({subscription_type})"
                )
            return success
        return False
    ...
    async def remove_user_subscription(
        ...
        if self.unsubscribe_callback:
            success = await self.unsubscribe_callback(
                symbol, user_id, subscription_type
            )
    ...
    def get_user_subscriptions(self, user_id: str) -> Set[Tuple[str, str]]:
        """Get all subscriptions for a specific user as (symbol, type) tuples"""
        return self.user_subscriptions.get(user_id, set())

    def get_user_symbols(self, user_id: str, subscription_type: str = None) -> Set[str]:
        ...
        if subscription_type:
            return {
                symbol
                for symbol, sub_type in subscriptions
                if sub_type == subscription_type
            }
        return {symbol for symbol, sub_type in subscriptions}

    def get_all_subscriptions(self) -> Dict[str, Set[Tuple[str, str]]]:
