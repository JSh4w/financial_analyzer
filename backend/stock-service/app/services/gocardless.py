...
class GoCardlessClient:
    ...
    async def get_requisition_details(self, requisition_id: str) -> dict:
        ...
        async with httpx.AsyncClient() as client:
            response = await client.get(
                f"https://bankaccountdata.gocardless.com/api/v2/requisitions/{requisition_id}/",
                headers={
                    "accept": "application/json",
                    "Authorization": f"Bearer {token}",
                },
        ...
        async with httpx.AsyncClient() as client:
            for account_id in account_ids:
                try:
                    ...
                    rate_limit_reset = int(response.headers.get("http_x_ratelimit_account_success_reset", 0))
                    rate_limit_remaining = int(response.headers.get("http_x_ratelimit_account_success_remaining", 0))
